...
public class ArMeasureActivity extends AppCompatActivity {
    ...
    private EditText et_front_width;
    private EditText et_side_depth;
...
//        overlayViewForTest = (OverlayView)findViewById(R.id.overlay_for_test);
        ...
        et_front_width = findViewById(R.id.et_front_width);
        et_side_depth = findViewById(R.id.et_side_depth);
        et_circumference = findViewById(R.id.et_circumference);
...
//                popUp.showAsDropDown(v, 0, 0); // show popup like dropdown list
        ...
        // Units
        ArrayAdapter<String> unitsAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item,
                new String[]{"cm","inch","ft"});
        spinner_units.setAdapter(unitsAdapter);
        spinner_units.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                try {
                    float cmFront = parseOrZero(et_front_width.getText().toString());
                    float cmSide = parseOrZero(et_side_depth.getText().toString());
                    float cmCirc = parseOrZero(et_circumference.getText().toString());
                    if(cmFront > 0) et_front_width.setText(String.format("%.1f", convertFromCm(cmFront)));
                    if(cmSide > 0) et_side_depth.setText(String.format("%.1f", convertFromCm(cmSide)));
                    if(cmCirc > 0) et_circumference.setText(String.format("%.1f", convertFromCm(cmCirc)));
                }catch (Exception ignored){}
            }
            @Override public void onNothingSelected(AdapterView<?> parent) { }
        });

        // Measurement flow buttons
    ...
    private enum MeasurePhase { IDLE, SCAN_BODY, FRONT_MEASURE, SIDE_MEASURE, DONE }
    private MeasurePhase phase = MeasurePhase.IDLE;
    private long phaseDeadlineMs = 0L;
    private boolean bodyDetected = false;

    private void startMeasurementFlow(){
        // Request camera permission if needed; renderer setup follows on permission grant
        tv_status.setText("Scanning for body...");
        phase = MeasurePhase.SCAN_BODY;
        phaseDeadlineMs = System.currentTimeMillis() + 15_000L;
        bodyDetected = false;
        
        // Show guidance overlay
        guidanceOverlay.setGuidance(GuidanceOverlay.GuidanceType.SCAN_BODY, 
                                   "Stand in the camera view", 0);
    }

    private void resetMeasurementFlow(){
        anchors.clear();
        showingTapPointX.clear();
        showingTapPointY.clear();
        phase = MeasurePhase.IDLE;
        tv_status.setText("Idle");
        tv_timer.setText("");
        et_front_width.setText("");
        et_side_depth.setText("");
        et_circumference.setText("");
        guidanceOverlay.hideGuidance();
    }

    private void updateTimerAndStatus(){
        long remain = Math.max(0, phaseDeadlineMs - System.currentTimeMillis());
        int sec = (int)Math.ceil(remain / 1000.0);
        tv_timer.setText(String.format("%02d:%02d", sec/60, sec%60));
        if(remain == 0){
            if(phase == MeasurePhase.SCAN_BODY && !bodyDetected){
                tv_status.setText("Timeout: No body detected. Please try again.");
                phase = MeasurePhase.IDLE;
            }else if(phase == MeasurePhase.FRONT_MEASURE){
                proceedToSide();
            }else if(phase == MeasurePhase.SIDE_MEASURE){
                finalizeMeasurements();
            }
        }
    }

    private void proceedToFront(){
        phase = MeasurePhase.FRONT_MEASURE;
        phaseDeadlineMs = System.currentTimeMillis() + 10_000L;
        tv_status.setText("Measure front width: place two points across the body");
        anchors.clear();
        showingTapPointX.clear();
        showingTapPointY.clear();
        
        // Show front pose guidance with countdown
        guidanceOverlay.setGuidance(GuidanceOverlay.GuidanceType.FRONT_POSE, 
                                   "Face the camera with arms outstretched", 10);
        startCountdown(10);
    }

    private void proceedToSide(){
        phase = MeasurePhase.SIDE_MEASURE;
        phaseDeadlineMs = System.currentTimeMillis() + 10_000L;
        tv_status.setText("Measure side depth: place two points (front to back)");
        anchors.clear();
        showingTapPointX.clear();
        showingTapPointY.clear();
        
        // Show side pose guidance with countdown
        guidanceOverlay.setGuidance(GuidanceOverlay.GuidanceType.SIDE_POSE, 
                                   "Turn sideways to the camera", 10);
        startCountdown(10);
    }

    private float convertFromCm(float cm){
        String unit = spinner_units.getSelectedItem() != null ? spinner_units.getSelectedItem().toString() : "cm";
        switch (unit){
            case "inch": return cm / 2.54f;
            case "ft": return cm / 30.48f;
            default: return cm;
        }
    }
    ...
    private void finalizeMeasurements(){
        ...
        tv_status.setText("Done. You can edit values or change units.");
        guidanceOverlay.hideGuidance();
        try {
            float front = parseOrZero(et_front_width.getText().toString());
            float side = parseOrZero(et_side_depth.getText().toString());
            if(front > 0 && side > 0){
                float a = front / 2f;
                float b = side / 2f;
                double c = Math.PI * (3*(a+b) - Math.sqrt((3*a + b)*(a + 3*b)));
                et_circumference.setText(String.format("%.1f", convertFromCm((float)c)));
            }
        }catch (Exception ignored){ }
    }
    ...
    private void shareMeasurements(){
        String unit = spinner_units.getSelectedItem() != null ? spinner_units.getSelectedItem().toString() : "cm";
        String msg = "Front: " + et_front_width.getText() + unit + "\n" +
                "Side: " + et_side_depth.getText() + unit + "\n" +
                "Circumference: " + et_circumference.getText() + unit;
        Intent sendIntent = new Intent();
        sendIntent.setAction(Intent.ACTION_SEND);
        sendIntent.putExtra(Intent.EXTRA_TEXT, msg);
        sendIntent.setType("text/plain");
        startActivity(Intent.createChooser(sendIntent, "Share measurements"));
    }
    
    private void startCountdown(int seconds) {
        if (seconds <= 0) return;
        
        guidanceOverlay.setGuidance(
            phase == MeasurePhase.FRONT_MEASURE ? GuidanceOverlay.GuidanceType.FRONT_POSE : GuidanceOverlay.GuidanceType.SIDE_POSE,
            phase == MeasurePhase.FRONT_MEASURE ? "Face the camera with arms outstretched" : "Turn sideways to the camera",
            seconds
        );
        ...
        mainHandler.postDelayed(() -> {
            if (seconds > 1) {
                ...
                guidanceOverlay.setGuidance(
                    phase == MeasurePhase.FRONT_MEASURE ? GuidanceOverlay.GuidanceType.FRONT_POSE : GuidanceOverlay.GuidanceType.SIDE_POSE,
                    "Ready to measure! Tap to place points",
                    0
                );
            }
        }, 1000);
    ...
    private void detectBodyWithPose(Frame frame) {
        // This is a simplified approach - in a real implementation, you'd capture
        // the camera frame and pass it to ML Kit pose detection
        // For now, we'll use the existing heuristic but with better validation
        ...
        // Enhanced heuristic: require both plane tracking and sufficient point cloud density
        boolean hasEnoughPoints = pointCloud.getPoints().remaining() > 300;
        boolean isTracking = frame.getCamera().getTrackingState() == TrackingState.TRACKING;
        
        if (hasPlane && hasEnoughPoints && isTracking) {
            bodyDetected = true;
            runOnUiThread(() -> {
                tv_status.setText("Body detected. Proceeding to front measurement...");
                proceedToFront();
            });
        }
    }
    ...
    protected void onResume() {
        ...
        if (session == null) {
            ...
            // Enable Depth if supported to improve scale estimation
            try {
...
//            log(TAG, "onDrawFrame(), mTouches.size=" + mTouches.size());
            ...
            try {
                ...
                // Enhanced body detection using ML Kit pose detection
                if(phase == MeasurePhase.SCAN_BODY && !bodyDetected){
                    detectBodyWithPose(frame);
                }

                // Update timer/status each frame
                if(phase != MeasurePhase.IDLE){
                ...
                if(anchors.size() < 1){
                    ...
                    // show result and capture into fields depending on phase
                    String unit = spinner_units.getSelectedItem() != null ? spinner_units.getSelectedItem().toString() : "cm";
                    float totalCm = (float)total;
                    float display = convertFromCm(totalCm);
                    String result = sb.toString().replaceFirst("[+]", "") + " = " + (((int)(display * 10f))/10f) + unit;
                    showResult(result);

                    if(anchors.size() >= 2){
                        if(phase == MeasurePhase.FRONT_MEASURE){
                            runOnUiThread(new Runnable() { @Override public void run() {
                                et_front_width.setText(String.format("%.1f", convertFromCm(totalCm)));
                            }});
                        }else if(phase == MeasurePhase.SIDE_MEASURE){
                            runOnUiThread(new Runnable() { @Override public void run() {
                                et_side_depth.setText(String.format("%.1f", convertFromCm(totalCm)));
                                finalizeMeasurements();
                            }});
                        }
                    }
